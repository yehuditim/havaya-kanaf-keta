...
const sceneHotspots: SceneHotspot[] = [
  ...
  { id: "distance", x: "20%", y: "25%", emoji: "📏", label: "מד מרחק", clue: "לוח נתונים: מרחק קו אווירי — 7,150 ק״מ! מספר עצום.", detail: "רשמו את נתון המרחק — הוא חיוני לחישוב בשלב הבא" },
  { id: "speed", x: "50%", y: "75%", emoji: "⚡", label: "מד מהירות", clue: "מהירות טיסה ממוצעת של העקב: 50 קמ״ש. לא מהר, אבל עקבי ובלי הפסקה!", detail: "בשילוב עם נתון המרחק — ניתן לגזור בנוסחה פשוטה כמה שעות ארך המסע" },
];
            ...
            <motion.div key="briefing" variants={phaseVariants} initial="initial" animate="animate" exit="exit" transition={phaseTransition} className="space-y-4">
              <div className="glass-card-immersive rounded-2xl p-5 station-glow-5">
                ...
                <NarrationPlayer
                  ...
                  audioSrc="/narration/station5-ringing.wav"
                  className="mb-3"
                />
            ...
            <motion.div key="explore" variants={phaseVariants} initial="initial" animate="animate" exit="exit" transition={phaseTransition}>
              ...
              {sceneComplete && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-30">
                  <p className="text-xs text-primary mb-2 bg-background/60 backdrop-blur-sm rounded-lg px-3 py-1 inline-block">✅ אספתם את כל הנתונים! עכשיו — חישוב</p>
                  <br />
                  <button onClick={() => { playClick(); setPhase("calculate"); }} className="bg-gradient-to-l from-secondary to-secondary/80 text-secondary-foreground px-6 py-2.5 rounded-xl font-black hover:scale-105 transition-all shadow-lg mt-1">
                    🧮 למשימת החישוב
                  </button>
                </div>
              )}
